...
Here we show both results of the iterated ESS and the proportion of of the full sample ISS for each sub-sampling case evaluated for either age or length composition sampling.
The proportion of full sample ISS was computed by pairing ISS as computed for the full dataset with no sub-sampling and the ISS for the dataset at the pre-determined sub-sampling level, and then dividing the sub-sampled data ISS by the full dataset ISS.
...
As number of samples taken for length frequency within hauls decreased, the iterated ESS decreased, but not drastically (Figure \@ref(fig:bs-length-ess), shown for EBS stocks as an example).
While we show the EBS stocks sampled in the 2021 shelf bottom trawl survey as an example, this trend of decreasing length composition iterated ESS with decreasing haul sub-sampling level for length frequency also resulted across all the years of the surveys evaluated, across all stocks evaluated, and across all sex categories evaluated.
There were three exceptions, however, for Greenland turbot and Kamchatka flounder in the EBS, and dover sole in the GOA, in which the iterated ESS was unaffected by the sub-sampling level. 
The result occurred because these stocks are not encountered in these surveys at high rates (Table \@ref(tab:species_sample)) and very few hauls had greater than fifty length frequency samples.
For all stocks evaluated the sex-specific iterated ESS (and subsequent ISS) were smaller than the total (combined sex) iterated ESS, this remained true within each individual sub-sampling case.
The magnitude of length composition iterated ESS (and subsequent ISS) was stock-specific and sex category-specific; across survey years the length composition ISS generally ranged from 250 to 5,000.
Across the length-freqeuncy haul level sub-sampling cases evaluated, the age composition iterated ESS for all stocks within each region was unaffected by length sub-sampling (Figure \@ref(fig:bs-age-length-ess), shown for EBS stocks as an example).

Similar to the results of length composition iterated ESS at different sub-sampling levels, as the sub-sampling level within a haul decreased the proportion of the full dataset ISS for the sub-sampled dataset ISS decreased (Figure \@ref(fig:prop-length-iss)).
This decrease in the proportion of length composition sub-sampled dataset ISS to full dataset ISS was generally consistent (with the exception of the stocks that were unaffected by sub-sampling as noted previously), although the magnitude of decrease was region and stock-specific.
While the magnitude of decrease in the proportion of length composition ISS was region and sex-specific, none of the stocks decreased to further than 50% of the full dataset ISS.
...
While the age composition iterated ESS (and subsequent ISS) was unaffected by the length frequency sub-sampling level, the iterated ESS did markedly decrease as the proportion of total age collections decreased (Figure \@ref(fig:bs-age-ess), shown for EBS stocks as an example); this result was consistent across all stocks and sex categories evaluated.
Similar to the results for length composition, the sex-specific iterated ESS and ISS were smaller than the total (combined sex) iterated ESS and ISS across all stocks evaluated.
Also, the magnitude of sex-specific and total iterated ESS and subsequent ISS were stock-specific, and ranged from 10 to 500; smaller than the length composition iterated ESS and ISS in the same year for the same stock.
The proportion of decrease from the full dataset ISS, as was the magnitude of ISS, was region and stock-specific, as was the variability in the decrease in ISS across the survey years (Figure \@ref(fig:prop-age-iss)).
For all the stocks evaluated, the proportion of decrease from the full dataset ISS was greater than 90% for a 90% decrease in the total age collections.
Further, for all stocks evaluated, the decrease in ISS was dampened when compared to the decrease in total age samples collected (Figure \@ref(fig:prop-age-iss-nss)).
For example, a reduction to 25% of the total age samples did not translate to a 25% reduction in the ISS, but rather resulted in a 30-40% reduction in ISS.
For all stocks and sex categories evaluated, the reduction in ISS was less than the reduction in the total age samples collected on the surveys.
...
```{r ss-saved}
knitr::kable(vroom::vroom(here::here('tables', 'ss_saved.csv')), caption = "Average total number of reductions in length frequency samples when sampling 100 and 150 fish per haul by stock and region evaluated.", align = c('lccc'), format.args = list(big.mark = ",", scientific = FALSE))
```
...
```{r bs-length-ess, fig.cap="Length composition interated effective sample size across length frequency sub-sampling levels evaluated for Eastern Bering Sea stocks sampled in the 2021 shelf bottom trawl survey."}
knitr::include_graphics(here::here('figs', 'bs_length_ess_examp.png'))
```

```{r bs-age-length-ess, fig.cap="Age composition interated effective sample size across length frequency sub-sampling levels evaluated for Eastern Bering Sea stocks sampled in the 2021 shelf bottom trawl survey."}
knitr::include_graphics(here::here('figs', 'bs_age_length_ess_examp.png'))
```
...
```{r bs-age-ess, fig.cap="Age composition interated effective sample size across total age collection sub-sampling levels evaluated for Eastern Bering Sea stocks sampled in the 2021 shelf bottom trawl survey."}
knitr::include_graphics(here::here('figs', 'bs_age_ess_examp.png'))
```
...
```{r prop-age-iss-nss, fig.cap="Proporion of reduction in input sample size (compared to full dataset input sample size) compared to prortion of reduction in total age sample size."}
