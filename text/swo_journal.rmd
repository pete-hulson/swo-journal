...
While the magnitude of decrease in the length composition relative ISS was survey region and sex-specific, none of the stocks decreased below 50%, and the majority of stocks did not decrease below 60% (Figure \@ref(fig:prop-length-iss)).
...
Additionally, the age composition ISS for gadid stocks were of similar magnitude an range in the AI and GOA, but were larger and had a greater range in the EBS.
...
In comparison, for gadids and other (Atka mackerel), the reduction in age composition relative ISS compared to reduction in specimen age samples was further dampened, where the reduction in total age samples resulted in a proportionally smaller reduction in age composition ISS compared to rockfish and flatfish stocks (this can be seen by comparing the slopes in the linear model fits in Figure \@ref(fig:age-ss-iss)).
...
In a similar study, @Siskey2023 evaluated the effect of increased age sampling beyond current sampling levels for a subset of stocks and also found that the age composition ISS continued to increase, up to age sampling levels 67% greater than current rates.
@Siskey2023 also found that, while holding the total age samples constant, improvements to age composition ISS were found by increasing sampling in more hauls even while decreasing the number of specimen age data collected in any given haul.
...
The results of our study, and ones like @Siskey2023, suggest that the number of age samples necessary to maximize to age composition information are likely beyond the current management agency capacities.
...
We also recommend (and iterate the results of @Siskey2023) that if sampling for ages were to be optimized for effort required by a survey team ISS can be increased by spreading these samples across more hauls while retaining the historical total number of age samples collected.
...
However, @Siskey2023 showed that there are downstream effects of increased uncertainty in assessment model estimates of the over fishing limit when age sampling is reduced.
