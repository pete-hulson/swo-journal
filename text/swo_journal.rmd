...
In this study, we developed a method to evaluate the consequences of reduced sampling effort on uncertainty in age and length composition data and demonstrated the proportional decrease in the ISS of reduced sampling datasets.
...
We find, based on the proportional decrease in ISS from sub-sampled data compared to full datasets, that there are recommendations that can be made to optimize age and length collections on fishery-independent platforms.
...
In practical implementation, their suggestion would result in species-specific sampling rates at the haul level, where larger fish would require more samples, and smaller fish would require less.
While this is a result that makes biological sense, and is attractive as it is related to life-history characteristics of the species being sampled, it could result in logistical difficulties when implemented 'on deck' during a survey.
We suggest, alternatively, that a more logistically feasible length frequency sampling method would be to set the same sampling rate across species at the haul level, recognizing that length frequiencies for some species may be over-sampled.
Here, we find that limiting sampling at 100 to 150 fish within a haul (whether for total or sex-specific length composition) would provide length composition data that have relatively similar uncertainty (within 10% of ISS) compared to length composition data derived from sampling at more intense rates.
...
This indicates as well that we are beyond a saturation point in the improvements made by any increased sampling effort in these surveys for length composition, however, this is not the case for age composition sampling.
...
As we increased the subsampling rates for age composition there was no 'leveling off' of age composition ISS that would indicate a saturation point had been met like that which resulted for length composition ISS. 
...
In order to determine whether and where age composition ISS reaches a saturation point simulation studies may be a more appropriate approach than bootstrapping historical data, as variables such as life-history or intra-haul correlation can be accounted for specifically.
...
Across the stocks evaluated, the decrease in length composition ISS as haul sub-sampling rates increased were extremely consistent, and no clear pattern emerged that could be related to life-history, however, there were interesting patterns that emerged when considering the effect of reduction in age sampling rates and the subsequent age composition ISS.
...
A similar result was found in @Hulson2017, where it was found that age sample size had a relatively larger effect on the uncertainty in age composition data for rockfish, then flatfish, then gadids (including Atka mackerel).
...
We show that, in comparison to the total (combined sex) age composition ISS the sex-specific age composition ISS is smaller, and between the two sexes there can be differences in the magnitude of ISS, often resulting in ISS that is larger for female age composition as compared to male.
...
Para on comparison with other survey reduction work (i.e., ICES WKUSER) - Meaghan

Para on sex and age sampling consequences to survey staff - Jason

Summary para that synthesizes all recommendations made - Pete
...
knitr::kable(vroom::vroom(here::here('tables', 'species_sample.csv')), 
             caption = "Average length and age (shown in parentheses) samples from the most recent three AFSC bottom trawl surveys by region for the species evaluated in the bootstrap-simulation for reduction in length and age collections.", 
             align = c('llccc'), 
